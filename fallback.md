...
- Windows `codex-auth login` accepts `codex.ps1` after `codex.exe` and `codex.cmd`.
  ...
  Removal conditions: remove this fallback once the supported Codex Windows launcher contract guarantees a single directly spawnable entry point, or once this repo intentionally narrows support to `codex.exe` and `codex.cmd` only.
